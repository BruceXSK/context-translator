...
- SES-006 [DONE] re-translate via the normal translate path: a re-translate goes through buildTranslateRequest with a `re-translate` marker leading the `<user-instruction>` block, appended to the committed prefix (`[system, ...turns, {user}]`) — the prefix stays stable (DS-001) and the message may duplicate an earlier paragraph's translation in the history. Pending context/instruction IS folded (so a just-added instruction applies to a re-translate) and the custom prompt is NOT re-folded (already in the segment's first user message, CFG-005). On success the user + assistant turns are appended via commitResponse (preserving the prefix for the next request), pending is cleared (a re-translate now consumes buffered context, like a fresh translation), the last-translation usage snapshot is updated, and cumulative usage accumulates. The separate re-translate request/commit path and its in-flight slot are removed — CT-018 serialization makes the single translate slot sufficient.
...
- **重翻译**（CT-016 / SES-006）：走普通翻译路径——`buildTranslateRequest(skeleton, 're-translate')` 把 `re-translate` 标识置于 `<user-instruction>` 块开头（并折入 pending context/instruction，故刚加的 instruction 会作用于重翻译），追加到已提交前缀末尾（`[system, ...turns, {user}]`，前缀稳定保 DS-001 缓存）；不重复折入 customPrompt（已在 segment 首条折入，CFG-005）；成功后经 `commitResponse` 追加 user+assistant 轮、更新 last 快照、累计 usage 并**清空 pending**（重翻译现在消费上下文，与全新翻译一致）。历史中可能存在该段的重复翻译条目（同一句多次重翻译）。
