...
- POP-006 [DONE] options page: a standalone settings page (open_in_tab) hosts base URL, API key, model, trigger key (a single-select of Alt/Shift/Ctrl, default Alt; stored as the standard KeyboardEvent.key name so the content-script key matcher hits), and system prompt; it reads/writes the same chrome.storage.local via the config module (shared schema with the popup), so changes take effect browser-wide — immediate for baseUrl/apiKey/model (the stateless service worker reads them per request), while trigger key and system prompt apply to pages loaded after the change (the content script snapshots them on load).
...
- 设置页（POP-006，独立 open_in_tab 页）：base URL / API key / model / 触发键 / system prompt。
