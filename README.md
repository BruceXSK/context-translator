...
- **OpenAI 兼容端点**：可接入任意 OpenAI 兼容的 chat completions 服务（如 DeepSeek、OpenAI 等），可自定义模型与自定义提示词（系统提示词为内置常量、不可改）。
...
- 保存即可。端点 / Key / 模型 / 思考模式 / 思考强度 / 最大上下文的改动**即时生效**；触发键与自定义提示词对**新加载的页面**生效（已打开的页面刷新一次即可）。
